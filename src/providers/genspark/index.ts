...
export function createGensparkAdapter(): ProviderAdapter {
  ...
  const tools: ToolDefinition[] = endpoint && token ? [
  ...
  return {
    ...
    async callTool(name, args, context) {
      if (!endpoint || !token || !tools.some((tool) => tool.name === name)) throw new GatewayError("PROVIDER_UNAVAILABLE", "Genspark worker is not configured.", 503);
